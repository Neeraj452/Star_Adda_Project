...
const DepositManual = () => {
  ...
  const profle = () => {
    ...
    axios
      .get(
        baseUrl +
          `txn/deposit/allmanual?page=${pageNumber}&_limit=${limit}&_q=${searchList}&_stype=${searchType}&_status=${findByStatus}`,
        { headers }
      )
      .then(res => {
  ...
  const checkupigatewayphonepe = (order_id, order_token, pay_date,msg=null) => {

    const access_token = localStorage.getItem("token");
    const headers = {
      Authorization: `Bearer ${access_token}`
    }
    axios.get(baseUrl + `phonpay-request-status?txnId=${order_id}`, { headers })
      .then((res) => {
        console.log(res.data, 'res.data')
        const icon = res.data.status == "PAID" ? "success" : "danger";
        const title = res.data.status == "PAID" ? "Deposit submited successfully" : "Transaction Failed";
  ...
  const checkupigatewaypaymanual = (order_id, order_token, pay_date,status) => {
  ...
  {data.User_id?.Phone ? (
    ...
    '-----'
                            ...
                            <td>
                             <span style={{color:"blue"}}>{data.order_token}</span> 
                           
                            </td>
                            <td>
                              <a  href={`${baseUrl}${data?.paymentImage}`} target="_blank">
                            <img
                              alt=''
...
<td>
                            ...
                            //&& (parseInt(gameCreatedAt) + 7200000) < currentTime
                              (data.status != 'PAID' && data.status != 'FAILED' ) ? <button className="btn btn-danger" onClick={() => universalCheckDeposit(data.payment_gatway, data.order_id,data._id, data.order_token, newdateFormat(data.createdAt))} >Approve</button> : 'Checked All'
                            }
                            ...
                            <td>
                              ...
                              {data.status != "FAILED" && data.status != "PAID" &&
                              <>
                               <button
                                className='btn btn-danger mr-2'
                                onClick={() => PaymanualReject(data.order_id)}
                              >
                                reject
                              </button>
                              </>
